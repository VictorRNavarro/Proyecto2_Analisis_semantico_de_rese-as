...
app.layout = dbc.Container(
    [
        html.H1("Análisis semántico de reseñas turísticas", className="mt-4"),
        html.P("Word2Vec, BETO y comparación de representaciones", className="text-muted"),
        dbc.Row([dbc.Col(dbc.Card(dbc.CardBody([html.Small(k), html.H3(v)])), md=3) for k, v in stats]),
        dcc.Tabs(
            id="tab",
            value="overview",
            className="mt-4",
...
def overview_layout():
    ...
    return dbc.Row([
        dbc.Col(dcc.Graph(figure=px.bar(ratings, x="calificacion", y="cantidad", title="Distribución de calificaciones")), md=6),
        dbc.Col(dcc.Graph(figure=px.pie(df, names="tipo_lugar", title="Reseñas por tipo de lugar")), md=6),
        dbc.Col(dcc.Graph(figure=px.bar(df.groupby("tipo_lugar", as_index=False)["calificacion"].mean(), x="tipo_lugar", y="calificacion", title="Calificación media por tipo")), md=12),
    ])
...
def word2vec_layout():
    ...
    if tsne_w2v is not None:
        ...
        components.append(dcc.Graph(figure=px.scatter(points, x="x", y="y", title="Word2Vec: proyección t-SNE de palabras")))
    else:
...
def beto_layout():
    ...
    if beto_tsne is not None:
        ...
        graph = px.scatter(beto_tsne, x="x", y="y", color=color, hover_data=[c for c in ["texto", "calificacion"] if c in beto_tsne], title="BETO: proyección t-SNE de reseñas")
    return html.Div([
